...
import com.lz.shiro.study.chapter6.JdbcTemplateUtils;
...
public class UserDaoImpl implements UserDao {
    ...
    private JdbcTemplate jt = JdbcTemplateUtils.jdbcTemplate();
    ...
    public void updateUser(User user) {
        ...
        jt.update(sql,user.getUsername(),user.getPassword(),user.getSalt(),user.getLocked(),user.getId());
    }
    ...
    private boolean exists(Long userId, Long roleId) {
        ...
        return jt.queryForObject(sql, Integer.class, userId, roleId) != 0;
    }

    public User findOne(Long userId) {
        String sql = "select id, username, password, salt, locked from sys_users where id=?";
        List<User> userList = jt.query(sql, new BeanPropertyRowMapper(User.class), userId);
        if(userList.size() == 0) {
            return null;
        }
    ...
    public User findByUsername(String username) {
        ...
        List<User> userList = jt.query(sql, new BeanPropertyRowMapper(User.class), username);
        if(userList.size() == 0) {
            return null;
        }
        return userList.get(0);
    ...
    public Set<String> findRoles(String username) {
        ...
        return new HashSet(jt.queryForList(sql, String.class, username));
    }
